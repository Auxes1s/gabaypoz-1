"""Official GabayPoz recommender v1.2.
...
* apply Q7 SHS strand as an aptitude multiplier,
* rank programs first,
...
Q12_MAP = {"A": 3, "B": 2, "C": 1}
STRAND_MULTIPLIERS: dict[str, dict[str, float]] = {
    "STEM": {"stem": 1.15},
    "ABM": {"business": 1.15},
    "HUMSS": {"arts": 1.15},
    "TVL": {"stem": 1.10, "agriculture": 1.10},
    "GAS": {},
    "Sports": {"arts": 1.10},
    "Arts and Design": {"arts": 1.10},
    "Sports / Arts and Design": {"arts": 1.10},
}
_STRAND_LOOKUP = {strand.lower(): multipliers for strand, multipliers in STRAND_MULTIPLIERS.items()}
Q12_PENALTY = 0.85
...
REQUIRED_QUESTIONS = SCORING_QUESTIONS | {"Q7", "Q10", "Q11", "Q12"}
...
def recommend_programs(
    *,
    ...
    """Return v1.2 recommendations and optionally persist the top three rows.
    ...
    try:
        ...
        student_vector, student_norm = _build_student_vector(responses, questions, warnings)
...
def _build_student_vector(
    ...
    aptitude = _apply_q7_strand_multiplier(responses["Q7"], aptitude, warnings)
...
def _apply_q7_strand_multiplier(
    strand: str,
    aptitude: np.ndarray,
    warnings: Optional[list[str]],
) -> np.ndarray:
    multipliers = _STRAND_LOOKUP.get(str(strand).strip().lower())
    if multipliers is None:
        if warnings is not None:
            _warn_once(warnings, "UNKNOWN_Q7_STRAND")
        multipliers = {}
    multiplier_vector = np.array([multipliers.get(dim, 1.0) for dim in DIMS], dtype=float)
    return np.clip(aptitude * multiplier_vector, 0.0, 1.0)
